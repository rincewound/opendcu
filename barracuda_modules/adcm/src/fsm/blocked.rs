...
use super::{
    emergency::Emergency, normal_operation::NormalOperation, DoorStateContainer, DoorStateImpl,
};
    ...
    fn dispatch_door_event(
        ...
        match d {
            DoorEvent::ValidDoorOpenRequestSeen(ap_id, token) => {
                commands.push(DoorCommand::ShowSignal(
                    ap_id,
                    barracuda_base_modules::sig::SigType::AccessDenied,
                ));
                commands.push(DoorCommand::TriggerEvent(
                    LogEvent::AccessDeniedDoorBlocked(passageway_id, token, ap_id),
                ));
            }
