...
use crate::{
    components::output_components::accessgranted::AccessGranted,
    components::{
        output_components::{alarmrelay::AlarmRelay, electricstrike::ElectricStrike},
        serialization_types::InputComponentSerialization,
        serialization_types::{OutputComponentSerialization, PassagewaySetting},
        InputComponent, OutputComponent, VirtualComponent,
    },
    DoorCommand, DoorEvent,
};
use barracuda_base_modules::{
    dcm::DoorOpenRequest,
    events::LogEvent,
    io::InputEvent,
    profile::{ProfileChangeEvent, ProfileState},
    sig::{SigCommand, SigType},
};
use barracuda_core::{
    core::shareable::Shareable,
    core::{broadcast_channel::GenericSender, channel_manager::ChannelManager, timer::Timer},
...
impl Passageway {
    fn load_input_components(
        components: &Vec<InputComponentSerialization>,
    ) -> Vec<Box<dyn InputComponent>> {
        ...
        for component in components.iter() {
            ...
            match component {
                InputComponentSerialization::FrameContact(setting) => {
                    the_object = Box::new(*setting)
                }
                InputComponentSerialization::DoorOpenerKey(dooropenerkey) => {
                    the_object = Box::new(*dooropenerkey)
                }
                InputComponentSerialization::DoorHandle(doorhandle) => {
                    the_object = Box::new(*doorhandle)
                }
                InputComponentSerialization::ReleaseContact(releasecontact) => {
                    the_object = Box::new(*releasecontact)
                }
            }
    ...
    fn load_output_components(
        components: &Vec<OutputComponentSerialization>,
        chm: &mut ChannelManager,
    ) -> Vec<Box<dyn OutputComponent>> {
        ...
        for component in components.iter() {
            ...
            match component {
                OutputComponentSerialization::ElectricStrike(setting) => {
                    the_object = Box::new(ElectricStrike::from_setting(*setting, chm))
                }
                OutputComponentSerialization::AccessGranted(setting) => {
                    the_object = Box::new(AccessGranted::from_setting(*setting, chm))
                }
                OutputComponentSerialization::AlarmRelay(output_id) => {
                    the_object = Box::new(AlarmRelay::new(*output_id, chm))
                }
            }
            deserialized_components.push(the_object);
        }
    ...
    fn find_release_time(settings: &PassagewaySetting) -> u64 {
        for i in settings.outputs.iter() {
            match i {
                OutputComponentSerialization::ElectricStrike(strike) => {
                    return strike.operation_time
                }
                _ => {}
            }
        }
    ...
    pub fn new(settings: PassagewaySetting, chm: &mut ChannelManager) -> Self {
        Self {
            ...
            output_components: Shareable::new(Passageway::load_output_components(
                &settings.outputs,
                chm,
            )),
            ...
            door_fsm: Shareable::new(DoorStateContainer::NormalOp(
                NormalOperation {},
                settings.id,
            )),
    ...
    pub fn apply_settings(&mut self, settings: PassagewaySetting) {
        ...
        self.output_components = Shareable::new(Passageway::load_output_components(
            &settings.outputs,
        ...
        if self.auto_switch_normal_timer.is_none() {
            self.auto_switch_normal_timer =
                Some(self.arm_timer(DoorEvent::DoorTimerExpired, self.release_time));
        }
    }
    ...
    fn inject_door_event(
        door_event: DoorEvent,
        current_door_state: &mut Shareable<DoorStateContainer>,
        generated_commands: &mut Vec<DoorCommand>,
    ) {
        let next_state: DoorStateContainer;
        let mut fsm_lcked = current_door_state.lock();
        let fsm = *fsm_lcked;
        match fsm {
            DoorStateContainer::NormalOp(op, id) => {
                next_state = op.dispatch_door_event(id, door_event, generated_commands);
            }
            DoorStateContainer::ReleasedOnce(op, id) => {
                next_state = op.dispatch_door_event(id, door_event, generated_commands);
            }
            DoorStateContainer::ReleasePerm(op, id) => {
                next_state = op.dispatch_door_event(id, door_event, generated_commands);
            }
            DoorStateContainer::Blocked(op, id) => {
                next_state = op.dispatch_door_event(id, door_event, generated_commands);
            }
            DoorStateContainer::Emergency(op, id) => {
                next_state = op.dispatch_door_event(id, door_event, generated_commands);
            }
        }
    ...
    fn arm_timer(&self, event_to_generate: DoorEvent, timeout: u64) -> Arc<bool> {
        ...
        return self.auto_event_timer.schedule(
            Box::new(move || {
                trace.trace(format!("Timer expired, generated: {}", event_to_generate));
                Passageway::inject_door_event(
                    event_to_generate,
                    &mut fsm_clone,
        ...
        for cmd in commands.iter() {
            match cmd {
                DoorCommand::ArmDoorOpenTooLongAlarm => {
                    self.auto_switch_normal_timer =
                        Some(self.arm_timer(DoorEvent::DoorOpenTooLong, self.alarm_time));
                }
                DoorCommand::DisarmDoorOpenTooLongAlarm => self.door_open_too_long_timer = None,
                DoorCommand::DisarmAutoswitchToNormal => self.auto_switch_normal_timer = None,
    ...
    fn do_doorcommands_for_outputs(
        outputs: &mut Shareable<Vec<Box<dyn OutputComponent>>>,
    ...
    pub fn on_door_open_request(&mut self, request: &DoorOpenRequest) {
        ...
        self.handle_door_event(DoorEvent::ValidDoorOpenRequestSeen(
            ...
            request.identification_token.clone(),
        ));
    }
