...
use barracuda_base_modules::{
    cfg::{self, cfgholder::FunctionType},
    dcm::DoorOpenRequest,
    events::LogEvent,
    io::{InputEvent, OutputState},
    modulebase::ModuleBase,
    profile::ProfileChangeEvent,
    sig::SigType,
};
use barracuda_core::core::{bootstage_helper::*, event::DataEvent};
use barracuda_core::core::{
    broadcast_channel::GenericReceiver, channel_manager::*, shareable::Shareable,
};
...
impl Display for DoorEvent {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            DoorEvent::ValidDoorOpenRequestSeen(apid, token) => {
                write!(f, "ValidDoorOpenRequestSeen({},{})", apid, token.len())
            }
            DoorEvent::Opened => write!(f, "Opened"),
            DoorEvent::Closed => write!(f, "Closed"),
            DoorEvent::DoorOpenProfileActive => write!(f, "DoorOpenProfileActive"),
            DoorEvent::DoorOpenProfileInactive => write!(f, "DoorOpenProfileInactive"),
            DoorEvent::BlockingContactEngaged => write!(f, "BlockingContactEngaged"),
            DoorEvent::BlockingContactDisengaged => write!(f, "BlockingContactDisengaged"),
            DoorEvent::ReleaseSwitchEngaged => write!(f, "ReleaseSwitchEngaged"),
            DoorEvent::ReleaseSwitchDisengaged => write!(f, "ReleaseSwitchDisengaged"),
            DoorEvent::DoorOpenerKeyTriggered => write!(f, "DoorOpenerKeyTriggered"),
            DoorEvent::DoorHandleTriggered => write!(f, "DoorHandleTriggered"),
            DoorEvent::DoorOpenTooLong => write!(f, "DoorOpenTooLong"),
            DoorEvent::DoorTimerExpired => write!(f, "DoorTimerExpired"),
        }
    }
}
...
impl ADCM {
    pub fn new(tracer: trace_helper::TraceHelper, chm: &mut ChannelManager) -> Self {
        ...
        for setting in result.storage.lock().iter() {
            result
                .passageways
                .push(Passageway::new(setting.clone(), chm));
        }
    ...
    pub fn init(&mut self) {
        ...
        let hli_cb = Some(|| {
            ...
            holder.register_handler(
                FunctionType::Put,
                "adcm/passageway".to_string(),
                Handler!(|pway: PassagewaySetting| {
    ...
    fn process_passageway_setting(
        passageway: PassagewaySetting,
        storage: &mut Shareable<JsonStorage<PassagewaySetting>>,
    ) {
    ...
    fn do_door_request(&mut self) {
        ...
        self.trace.trace(format!(
            "DoorRequest for accesspoint {}",
            door_request.access_point_id
        ));
        for passageway in self.passageways.iter_mut() {
    ...
    fn update_passageway(&mut self, pway_id: u32) {
        if let Some(setting) = self.storage.lock().get_entry(|x| x.id == pway_id) {
            if let Some(pway) = self.passageways.iter_mut().find(|x| x.id == pway_id) {
                ...
                self.passageways
                    .push(Passageway::new(setting.clone(), &mut self.channel_manager));
            }
        }
    }
