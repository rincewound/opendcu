...
use barracuda_base_modules::modcaps::{
    ModCapAggregator, ModuleCapabilityAdvertisement, ModuleCapabilityType,
};
use barracuda_base_modules::{acm::*, sig::*};
use barracuda_base_modules::{
    cfg::{self, cfgholder::*, ConfigMessage},
    Handler,
};
use barracuda_core::core::broadcast_channel::*;
use barracuda_core::core::channel_manager::*;
use barracuda_core::core::{bootstage_helper::*, shareable::Shareable, SystemMessage};
use barracuda_core::trace::*;
use std::thread;

use profiles::{AccessProfile, JsonProfileChecker, ProfileChecker};
...
where
    T: whitelist::WhitelistEntryProvider + std::marker::Send,
{
    let tracer = trace_helper::TraceHelper::new("ACM/Whitelist".to_string(), chm);
    let mut wl = GenericWhitelist::new(
        tracer,
        chm,
        T::new(),
        JsonProfileChecker::new("profiles.txt".to_string()),
    );
    thread::spawn(move || {
        wl.init();
        loop {
...
struct GenericWhitelist<
    WhitelistProvider: whitelist::WhitelistEntryProvider,
    ProfileStorage: ProfileChecker,
> {
...
impl<
        WhitelistProvider: whitelist::WhitelistEntryProvider + Send + 'static,
        ProfileStorage: ProfileChecker + Send + 'static,
    > GenericWhitelist<WhitelistProvider, ProfileStorage>
{
    fn new(
        trace: trace_helper::TraceHelper,
        chm: &mut ChannelManager,
        whitelist: WhitelistProvider,
        profile_source: ProfileStorage,
    ) -> Self {
        GenericWhitelist {
            tracer: trace,
    ...
    pub fn init(&mut self) {
        ...
        let hli_cb = Some(|| {
            ...
            */
            ...
            holder.register_handler(
                FunctionType::Put,
                "wl/entry".to_string(),
                Handler!(|r: whitelist::WhitelistEntry| {
            ...
            holder.register_handler(
                FunctionType::Put,
                "wl/profile".to_string(),
                Handler!(|newprofile: AccessProfile| {
            ...
            holder.register_handler(
                FunctionType::Delete,
                "wl/profile".to_string(),
                Handler!(|profile_to_delete: AccessProfile| {
    ...
    fn process_access_request(&self, req: WhitelistAccessRequest) {
        ...
        if let Some(entry) = entry {
            if let Ok(sud_ap_id) = self
                .modcaps
                .sud_to_logical_id(req.access_point_id, ModuleCapabilityType::AccessPoints)
            {
                if !self.check_profile(sud_ap_id, &entry) {
                    return;
                }

                // Good? If so, emit DoorOpenRequest, otherwise emit AccessDenied Sig
                self.tracer.trace(format!(
                    "Request seems ok for token {:?}, sending door open request.",
                    entry.identification_token_id.clone()
                ));
                let openreq = DoorOpenRequest {
                    access_point_id: sud_ap_id,
                    identification_token: entry.identification_token_id,
                };
                self.door_tx.send(openreq);
            } else {
                self.tracer.trace(format!(
                    "Received access request from unknown accesspoint {}",
                    req.access_point_id
                ));
            }
        } else {
            self.tracer
                .trace_str("Access Denied; Unknown identification token.");
    ...
    fn process_delete_entry_req(
        wl: &Shareable<WhitelistProvider>,
    ...
    fn process_put_profile_req(
        current_profiles: &Shareable<ProfileStorage>,
...
#[cfg(test)]
mod tests {
    use crate::profiles::{AccessProfile, ProfileCheckResult, ProfileChecker};
    ...
    use barracuda_base_modules::{
        acm::WhitelistAccessRequest, modcaps::ModuleCapabilityAdvertisement, sig::*,
    };
    ...
    impl crate::whitelist::WhitelistEntryProvider for DummyWhitelist {
        ...
        fn get_entry(
            &self,
            _identity_token_id: Vec<u8>,
        ) -> Option<crate::whitelist::WhitelistEntry> {
            self.entry.clone()
        }
    ...
    impl ProfileChecker for DummyProfileChecker {
        ...
        fn get_profile(&self, _profile_id_: u32) -> Option<AccessProfile> {
            None
        }
    ...
    fn make_whitelist(
        chm: &mut ChannelManager,
    ) -> crate::GenericWhitelist<DummyWhitelist, DummyProfileChecker> {
        let wl = DummyWhitelist::new();
        let prof = DummyProfileChecker {
            check_result: Ok(()),
        };
        let tracer = trace_helper::TraceHelper::new("ACM/Whitelist".to_string(), chm);
    ...
    fn will_generate_door_open_request_if_token_is_known_and_access_rights_are_good() {
        ...
        let mut md = crate::GenericWhitelist::new(
            tracer,
            &mut chm,
            wl,
            DummyProfileChecker {
