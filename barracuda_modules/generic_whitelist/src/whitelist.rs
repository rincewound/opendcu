...
impl WhitelistEntryProvider for JsonEntryProvider {
    ...
    fn get_entry(&self, identity_token_id: Vec<u8>) -> Option<WhitelistEntry> {
        return self
            .entries
            .get_entry(|x| x.identification_token_id.cmp(&identity_token_id) == Ordering::Equal);
    }

    fn put_entry(&mut self, entry: WhitelistEntry) {
        // delete entry if already existing..
        self.entries.delete_entry(|x| {
            x.identification_token_id
                .cmp(&entry.identification_token_id)
                == Ordering::Equal
        });
    ...
    fn delete_entry(&mut self, identity_token_id: Vec<u8>) {
        self.entries
            .delete_entry(|x| x.identification_token_id.cmp(&identity_token_id) != Ordering::Equal);
        self.entries.update_storage();
    }
}
