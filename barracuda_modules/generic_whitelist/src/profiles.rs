    ...
    T: chrono::TimeZone,
{
    ...
    if !profile.access_points.contains(&ap_id) {
        return Err(ProfileCheckResult::NoAccessRights);
    }
...
#[cfg(test)]
mod tests {
    use super::{check_profile_impl, AccessProfile, TimeSlot};
    use chrono::DateTime;
    ...
    fn check_profile_yields_true_if_valid_profile() {
        let profile = AccessProfile {
            id: 0,
            access_points: vec![1, 2],
            time_pro: vec![TimeSlot {
                day: super::Weekday::Monday,
                from: 700,
                to: 1000,
            }],
        };
