...
impl ModCapAggregator {
    ...
    pub fn add_message(&mut self, message: ModuleCapabilityAdvertisement) {
        ...
        for cap in message.caps.iter() {
            match cap {
                ModuleCapability::Inputs(num_in) => {
                    Self::add_cap(&mut self.inputs, message.module_id, *num_in)
                }
                ModuleCapability::Outputs(num_out) => {
                    Self::add_cap(&mut self.outputs, message.module_id, *num_out)
                }
                ModuleCapability::AccessPoints(num_ap) => {
                    Self::add_cap(&mut self.access_points, message.module_id, *num_ap)
                }
                ModuleCapability::_KeypadEntry(num_kp) => {
                    Self::add_cap(&mut self.inputs, message.module_id, *num_kp)
                }
            }
        }
    }
...
mod tests {
    ...
    fn make_message(mod_id: u32, cap: ModuleCapability) -> ModuleCapabilityAdvertisement {
        return ModuleCapabilityAdvertisement {
            module_id: mod_id,
            caps: vec![cap],
        };
    }
    ...
    pub fn will_yield_correct_module_id() {
        ...
        let result = a
            .logical_id_to_sud(13, ModuleCapabilityType::Inputs)
            .unwrap();
        assert_eq!(result, 0x05000003);

        let result2 = a
            .logical_id_to_sud(2, ModuleCapabilityType::Inputs)
            .unwrap();
    ...
    pub fn can_convert_sud_to_logical() {
        ...
        let result = a
            .sud_to_logical_id(0x04000003, ModuleCapabilityType::Inputs)
            .unwrap();
        assert_eq!(result, 3);
        let result = a
            .sud_to_logical_id(0x05000003, ModuleCapabilityType::Inputs)
            .unwrap();
        assert_eq!(result, 13);
    }
