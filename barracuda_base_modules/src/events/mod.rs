...
use barracuda_core::{
    core::{
        bootstage_helper::boot_noop, broadcast_channel::GenericReceiver,
        channel_manager::ChannelManager, event::DataEvent, shareable::Shareable,
    },
    select_chan,
    trace::trace_helper::{self, TraceHelper},
    wait_for,
};

use crate::{
    cfg::{self, ConfigMessage},
...
impl EventStore {
    ...
    pub fn init(&self) {
        ...
        let hli_cb = Some(|| {
            ...
            holder.register_read_handler(
                "events".to_string(),
                ReadDataHandler!(|| {
                    let mut data_access = event_buffer_access.lock();
                    let items_to_take = if data_access.len() < 20 {
                        data_access.len()
                    } else {
                        20
                    };
