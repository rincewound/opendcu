...
use barracuda_core::util::{datetime::TimeSlot, JsonStorage, ObjectStorage};
...
impl ProfileChecker {
    ...
    fn is_profile_active(prof: &BinaryProfile, time: chrono::DateTime<Local>) -> bool {
        for slot in prof
            .time_slots
            .iter()
            .filter(|x| x.day as u32 == time.weekday() as u32)
        {
    ...
    pub fn tick(
        ...
        last_time: chrono::DateTime<Local>,
    ) -> Vec<ProfileChangeEvent> {
        ...
        for profile in self.profiles.iter() {
            ...
            if (is_active_now != was_profile_active) || !self.first_check_done {
                let evt = ProfileChangeEvent {
                    ...
                    profile_state: if is_active_now {
                        super::ProfileState::Active
                    } else {
                        super::ProfileState::Inactive
                    },
                };
                result.push(evt);
            }
