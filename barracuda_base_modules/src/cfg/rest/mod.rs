...
impl ConfigRest {
    ...
    pub fn init(&mut self) {
        ...
        let hlicb = Some(move || the_sender.send(super::ConfigMessage::RegisterHandlers(the_cfg)));
    ...
    fn do_put(&self, req: &rouille::Request, _module: String) -> rouille::Response {
        ...
        self.cfg.lock().do_put(_module, reqdata);
    ...
    fn do_post(&self, req: &rouille::Request, _module: String) -> rouille::Response {
        ...
        let mut d = req.data().unwrap();
        let _ = d.read_to_end(&mut reqdata);
        self.cfg.lock().do_post(_module, reqdata);
        rouille::Response::text("ok").with_status_code(200)
    }
    ...
    fn do_get(&self, req: &rouille::Request, _module: String) -> rouille::Response {
        ...
        let mut d = req.data().unwrap();
        let _ = d.read_to_end(&mut reqdata);
        let response = self.cfg.lock().do_get(_module);
        match response {
            Ok(data) => {
                rouille::Response::from_data("application/octet-stream", data).with_status_code(200)
            } // Ok
            Err(CfgError::ResourceEmpty) => {
                rouille::Response::text("Resource empty").with_status_code(406)
            } // Not Acceptable
            Err(CfgError::ResourceNotFound) => {
                rouille::Response::text("Resource not found").with_status_code(404)
            } // Not found
        }
    }
    ...
    fn do_delete(&self, req: &rouille::Request, _module: String) -> rouille::Response {
        ...
        let mut d = req.data().unwrap();
        let _ = d.read_to_end(&mut reqdata);
        self.cfg.lock().do_delete(_module, reqdata);
        rouille::Response::text("ok").with_status_code(200)
    }
