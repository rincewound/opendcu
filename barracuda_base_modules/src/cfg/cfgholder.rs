...
impl CfgHolder {
        ...
        F: FnMut(Vec<u8>) -> () + Send,
    {
        ...
        self.write_funcs.lock().insert(key, Box::new(func));
    }
    ...
    where
        F: FnMut() -> Vec<u8> + Send,
    {
        self.read_funcs.lock().insert(route, Box::new(func));
    }
    ...
    fn do_action(&mut self, action: FunctionType, route: String, data: Vec<u8>) {
        ...
        let item = self.write_funcs.lock().remove_entry(&the_key);
        if item.is_none() {
            return;
        }
        ...
        self.write_funcs.lock().insert(the_key, func);
...
mod tests {
    ...
    /*************  ✨ Codeium Command ⭐  *************/
    /// Tests that calling do_put triggers the correct function
    ///
    /// In this test, we register a function that sets a boolean to true
    /// when called. We then call do_put and check that the boolean has
    /// been set to true.
    ///
    /******  253fd6a2-8c89-4fcf-8a36-73f173ae10b5  *******/
    #[test]
    ...
    pub fn get_returns_value() {
        let mut hdl = CfgHolder::new();
        hdl.register_read_handler("cfg/foo".to_string(), move || return vec![1, 2, 3, 4]);
