...
impl Supervisor {
    ...
    where
        T: FnOnce(&mut crate::core::channel_manager::ChannelManager),
    {
        self.num_threads += 1;
    ...
    fn wait_for_stage_completion(
        &self,
        recv: &GenericReceiver<SystemMessage>,
        stage: BootStage,
                    ...
                    SystemMessage::StageComplete(the_stage, mod_id) => {
                        if std::mem::discriminant(&the_stage) == std::mem::discriminant(&stage) {
                            ...
                            self.tracer.trace(format!(
                                "Module {}, instance {} checked in for stage {} ",
                                mod_type, mod_instance, the_stage as u32
                            ));
        ...
        if messages_left > 0 {
            let msg = format!(
                "{} modules failed to run stage {}",
                messages_left, stage as u32
            );
            println!("{}", msg);
