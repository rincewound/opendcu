...
pub trait ObjectStorage<T> {
    fn get_entry<P>(&self, filter: P) -> Option<T>
    where
        P: FnMut(&T) -> bool;
    fn put_entry(&mut self, entry: T);
    fn delete_entry<P>(&mut self, filter: P)
    where
        P: FnMut(&T) -> bool;
...
impl<ValueType> JsonStorage<ValueType>
where
...
impl<ValueType> ObjectStorage<ValueType> for JsonStorage<ValueType>
where
    ValueType: Clone + Serialize + DeserializeOwned,
{
    fn get_entry<P>(&self, mut filter: P) -> Option<ValueType>
    where
    ...
    fn delete_entry<P>(&mut self, mut filter: P)
    where
        P: FnMut(&ValueType) -> bool,
    {
